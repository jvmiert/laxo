...
func (a *Activities) SaveLazadaProducts(ctx context.Context, param LazadaSaveParam) error {
	...
	laxoP, err := a.shopService.SaveOrUpdateProductToStore(product, param.ShopID, pModel.ID, param.Overwrite)
	if err != nil {
	...
	if strings.ToLower(pModelSKU.Status.String) != "suspended" {
		...
		err = a.assetsService.SaveAndAssignImagesFromURL(images, param.ShopID, laxoP.Model.ID, shop.Model.AssetsToken)
		if err != nil {
